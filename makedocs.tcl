...
[subsection "BUILTIN METHODS"]
	...
	[def "Result Formatting Operators"]
		...
		[cmd_def "base32 [arg [opt options]] [arg schema]"]
			[list_begin definitions]
			[def "for collecting"]
				Validates the decoded JSON input with the [arg schema]. That result is then
			...
			[def "for composing"]
				Passes the Tcl input data into Tcllib's [cmd "::base32::encode"] command.
				[para]
 				The result of that is then validated with the [arg schema].
				The operator returns the result of the schema.
			...
			[emph "Note:"] Tcllib's [const "base32"] package must be loaded before
			collecting/composing with any schema using this method.

		[cmd_def "base64 [arg [opt options]] [arg schema]"]
			[list_begin definitions]
			[def "for collecting"]
				Validates the decoded JSON input with the [arg schema]. That result is then
		...
		[cmd_def "json [arg [opt options]] [arg schema]"]
			[list_begin definitions]
			[def "for collecting"]
				Validates the decoded JSON input with the [arg schema]. That result is then
				expanded and passed into xjson's [cmd "::xjson::decode"] command.
				[para]
				The operator returns the result of xjson's [cmd "::xjson::decode"] command.

			[def "for composing"]
				Passes the Tcl input data into xjson's [cmd "::xjson::encode"] command.
				[para]
 				The result of that is then validated with the [arg schema].
				The operator returns the result of the schema.
		...
		[cmd_def "mark [arg [opt options]] [arg mark] [arg schema]"]
			[list_begin definitions]
			[def "for collecting"]
