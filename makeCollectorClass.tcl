...
##
## Factory for creating JSON validator and data collector classes.
##
...
## Create a class that validates decoded JSON data against a schema, collects the data
## and transforms it according to the types and operators used in said schema.
proc ::xjson::makeCollectorClass {args} {
	...
	for {set index 0} {$index < [llength $args]} {incr index} {
		...
		## Fail.
		return -code error -errorcode {XJSON COLLECTOR FACTORY UNKNOWN_OPTION} \
			[string cat "wrong args: should be \"::xjson::makeCollectorClass ?-nobuiltins? ?-tabulator value? ?-maxldepth value? ?-maxweight value? ?-maxlines value?  ?--? className ?methodName methodDefinition ...?\""]
	}

	## Fail on missing class name.
	if {[llength $argvalues]%2!=1} {
		return -code error -errorcode {XJSON COLLECTOR FACTORY WRONGARGS}
	}
	...
	if {![dict exists $options "-nobuiltins"]} {
		set definitions $::xjson::builtinCollectingMethods
	} else {
		set definitions {}
	}
	...
	lassign [::xjson::_compileMethods COLLECTOR $definitions] methodScripts methodConsts
	...
	## Add compiled collecting method definitions.
	lappend classbody [list protected common _collectingMethods $methodConsts]
	...
	foreach {option is default} {
		...
		if {[dict exists $options $option]} {
			...
			if {$is ne {} && ![string is $is [dict get $options $option]]} {
				return -code error -errorcode {XJSON COLLECTOR FACTORY OPTION_TYPE_MISMATCH} \
	...
	lappend classbody {
		...
		protected proc _printData {data} {
			::xjson::_printJsonData $data $_tabulator $_maxlines
		}
		...
		protected proc _printSchema {parsedschema {indent 0}} {
			::xjson::_printSchema $parsedschema $indent $_tabulator $_maxdepth $_maxweight $_maxldepth $_collectingMethods
		}
		...
		protected proc _parseSchema {schema dubious {path /} {depth 0} {ldepth 0} {weight 0}} {
			::xjson::_parseSchema $schema $dubious $path $depth $ldepth $weight $_tabulator $_collectingMethods
		}
		...
		## Nested collectors registered in this object.
		protected variable _nestedCollectors [dict create]
		...
		constructor {args} {
			...
			for {set index 0} {$index < [llength $args]} {incr index} {
				...
				if {$element in {-init -recursionlimit}} {
					...
					if {$index == [llength $args]} {
						return -code error -errorcode {XJSON COLLECTOR CLASS WRONGARGS} \
							[string cat "wrong args: should be \"" [info class] " objectName " \
								"?-trusted? ?-init body? ?-recursionlimit limit? ?--? ?nestedCollectorName nestedCollectorObject ...? schema\""]
					}
				...
				## Fail.
				return -code error -errorcode {XJSON COLLECTOR CLASS UNKNOWN_OPTION} \
					[string cat "wrong args: should be \"" [info class] " objectName " \
						"?-trusted? ?-init body? ?-recursionlimit limit? ?--? ?nestedCollectorName nestedCollectorObject ...? schema\""]
			}
			...
			## Remember the nested collectors.
			foreach {name object} [lrange $argvalues 0 end-1] {
				dict set _nestedCollectors $name $object
			}
		...
		protected method _collect {data schema path interpreter previous} {
			...
			## Continue with the collecting method in question.
			*[dict get $_collectingMethods [dict get $schema method] method] $data $schema $path $interpreter $previous
		}


		## Validate and collect data from JSON data to Tcl data. This wrapper method for
		## the internal [_collect] method uses the schema that is stored in this object.
		public method collect {data {path /}} {
			...
			if {[catch {$interpreter eval [list apply [list {} $_initscript]]} result]} {
				...
				return -code error -errorcode $::errorCode -errorinfo $::errorInfo \
					[string cat "Collect initialization failed at " $path ":\n" $result]
			}

			## Collect data.
			if {[catch {_collect $data $_schema $path $interpreter {}} collected]} {
				## On error, delete the interpreter.
				interp delete $interpreter
